...
class LinearHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    public void addElement(K key, V value)
    {
        ...
        while(_items.elementAt(hash)!= null && _items.elementAt(hash) != key){     // check to see if index is empty if not iterate
            hash++;
            hash %= _items.size();
        }
        _items.setElementAt(_items.elementAt(hash), hash);
